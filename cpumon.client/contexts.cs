...
sealed class AgentContext : ApplicationContext
{
    ...
    void HandlePawPayload(ServerCommand cmd)
    {
        ...
        else if (cmd.Cmd == "paw_services" && cmd.PawSource != null && cmd.PawServices != null) { var src = cmd.PawSource; var svcs = cmd.PawServices; _uiCtx.Post(_ => _pawForm?.ReceiveServiceList(src, svcs), null); }
...
sealed class DaemonContext : ApplicationContext
{
    ...
    void SendCpuDetail(string? cmdId)
    {
        ...
        {
            LogSink.Warn("Daemon.CpuDetail", "Failed to send CPU detail", ex);
        }
    }
