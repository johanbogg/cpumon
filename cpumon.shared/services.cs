...
public static class SysInfoCollector
{
    ...
    {
        var info = new SystemInfoReport { Hostname = Environment.MachineName, Domain = Environment.UserDomainName, UserName = Environment.UserName, DotNetVersion = Environment.Version.ToString(), UptimeHours = Environment.TickCount64 / 3600000.0, OsBuild = Environment.OSVersion.ToString(), CpuCores = Environment.ProcessorCount, CpuThreads = Environment.ProcessorCount };
        var tOs      = Task.Run(() => Wmi("Win32_OperatingSystem", "Caption"));
        var tCpu     = Task.Run(() => Wmi("Win32_Processor", "Name"));
        var tGpu     = Task.Run(() => Wmi("Win32_VideoController", "Name"));
        var tCores   = Task.Run(() => Wmi("Win32_Processor", "NumberOfCores"));
        var tThreads = Task.Run(() => Wmi("Win32_Processor", "NumberOfLogicalProcessors"));
        var tRam     = Task.Run(() => { try { using var q = new ManagementObjectSearcher("SELECT TotalVisibleMemorySize,FreePhysicalMemory FROM Win32_OperatingSystem"); foreach (var o in q.Get()) { double tot = o["TotalVisibleMemorySize"] != null ? Convert.ToDouble(o["TotalVisibleMemorySize"]) / 1048576.0 : 0; double avl = o["FreePhysicalMemory"] != null ? Convert.ToDouble(o["FreePhysicalMemory"]) / 1048576.0 : 0; return (tot, avl); } } catch { } return (0.0, 0.0); });
        Task.WaitAll(new Task[] { tOs, tCpu, tGpu, tCores, tThreads, tRam }, 5000);
        info.OsName  = tOs.IsCompletedSuccessfully  ? tOs.Result  : "";
        info.CpuName = tCpu.IsCompletedSuccessfully ? tCpu.Result : "";
        info.GpuName = tGpu.IsCompletedSuccessfully ? tGpu.Result : "";
        if (tCores.IsCompletedSuccessfully   && int.TryParse(tCores.Result,   out int c)) info.CpuCores   = c;
        if (tThreads.IsCompletedSuccessfully && int.TryParse(tThreads.Result, out int t)) info.CpuThreads = t;
        if (tRam.IsCompletedSuccessfully && tRam.Result.Item1 > 0) { info.RamTotalGB = tRam.Result.Item1; info.RamAvailGB = tRam.Result.Item2; }
...
public static class FileBrowserService
{
    ...
    public static void SendFile(string filePath, string transferId, string? cmdId, object netLock, StreamWriter? writer) { Task.Run(() => { try { var fi = new FileInfo(filePath); if (!fi.Exists) { var err = new ClientMessage { Type = "file_chunk", TransferId = transferId, CmdId = cmdId, FileChunk = new FileChunkData { TransferId = transferId, FileName = System.IO.Path.GetFileName(filePath), Error = "File not found", IsLast = true } }; lock (netLock) { writer?.WriteLine(JsonSerializer.Serialize(err, Proto.JsonOpts)); writer?.Flush(); } return; } long total = fi.Length; long offset = 0; var buf = new byte[Proto.FileChunkSize]; using var fs = fi.OpenRead(); while (true) { int read = fs.Read(buf, 0, buf.Length); bool last = read == 0 || offset + read >= total; var chunk = new FileChunkData { TransferId = transferId, FileName = fi.Name, Data = read > 0 ? Convert.ToBase64String(buf, 0, read) : "", Offset = offset, TotalSize = total, IsLast = last }; var msg = new ClientMessage { Type = "file_chunk", TransferId = transferId, CmdId = cmdId, FileChunk = chunk }; lock (netLock) { writer?.WriteLine(JsonSerializer.Serialize(msg, Proto.JsonOpts)); writer?.Flush(); } offset += read; if (last) break; } } catch (Exception ex) { var err = new ClientMessage { Type = "file_chunk", TransferId = transferId, CmdId = cmdId, FileChunk = new FileChunkData { TransferId = transferId, Error = ex.Message, IsLast = true } }; lock (netLock) { try { writer?.WriteLine(JsonSerializer.Serialize(err, Proto.JsonOpts)); writer?.Flush(); } catch { } } } }); }
...
public sealed class TerminalSession : IDisposable
{
    ...
    void Emit(string text) { if (_disposed) return; var msg = new ClientMessage { Type = "terminal_output", TermId = Id, Output = text }; lock (_netLock) { try { _netWriter?.WriteLine(JsonSerializer.Serialize(msg, Proto.JsonOpts)); _netWriter?.Flush(); } catch { } } }
    void EmitClosed() { if (_disposed) return; var msg = new ClientMessage { Type = "terminal_closed", TermId = Id }; lock (_netLock) { try { _netWriter?.WriteLine(JsonSerializer.Serialize(msg, Proto.JsonOpts)); _netWriter?.Flush(); } catch { } } }
...
public static class CmdExec
{
    ...
    static void RunInner(ServerCommand cmd, object lk, ref StreamWriter? wr)
    {
        switch (cmd.Cmd)
        {
            ...
            case "listprocesses": {
                ...
                Task.Run(() => {
                    try {
                        ...
                        lock (lk) { wrSnap?.WriteLine(JsonSerializer.Serialize(m2, Proto.JsonOpts)); wrSnap?.Flush(); }
                    } catch { }
                });
                break;
            }
            ...
            case "sysinfo": try { var si = SysInfoCollector.Collect(); var m2 = new ClientMessage { Type = "sysinfo", SysInfo = si, CmdId = cmd.CmdId }; lock (lk) { wr?.WriteLine(JsonSerializer.Serialize(m2, Proto.JsonOpts)); wr?.Flush(); } } catch (Exception ex) { Res(cmd.CmdId, false, ex.Message, lk, wr); } break;
            ...
            case "screenshot": { var shot = ScreenshotService.Capture(cmd.CmdId, 80, cmd.RdpMonitorIndex); var sm = new ClientMessage { Type = "screenshot", Screenshot = shot, CmdId = cmd.CmdId }; lock (lk) { wr?.WriteLine(JsonSerializer.Serialize(sm, Proto.JsonOpts)); wr?.Flush(); } break; }
            case "file_list": try { var listing = FileBrowserService.ListDirectory(cmd.Path); var flMsg = new ClientMessage { Type = "file_listing", FileListing = listing, CmdId = cmd.CmdId }; lock (lk) { wr?.WriteLine(JsonSerializer.Serialize(flMsg, Proto.JsonOpts)); wr?.Flush(); } } catch (Exception ex) { Res(cmd.CmdId, false, $"List: {ex.Message}", lk, wr); } break;
            case "file_download": if (cmd.Path != null && cmd.TransferId != null) FileBrowserService.SendFile(cmd.Path, cmd.TransferId, cmd.CmdId, lk, wr); break;
            ...
            case "file_upload_abort": if (cmd.TransferId != null && ActiveUploads.TryRemove(cmd.TransferId, out var aborted)) { string tmp = aborted.Name; try { aborted.Dispose(); } catch { } try { File.Delete(tmp); } catch { } } break;
    ...
    static void Res(string? id, bool ok, string msg, object lk, StreamWriter? wr)
    {
        ...
        lock (lk) { try { wr?.WriteLine(JsonSerializer.Serialize(m, Proto.JsonOpts)); wr?.Flush(); } catch { } }
    }
...
public sealed class HardwareMonitorService : IDisposable
{
    ...
    public CpuSnapshot GetSnapshot(bool includeCores = false) { if (_ck.ElapsedMilliseconds >= 400) { Upd(); _ck.Restart(); } if (_h == null && !_wf) return CpuSnapshot.Unavailable(); float? t = RV(_ts), f = RV(_cs), l = RV(_ls), p = RV(_ps); if ((f == null || f <= 0) && _pf != null) { try { float pct = _pf.NextValue(); float n = Nom(); if (n > 0 && pct > 0) f = n * pct / 100f; } catch { } } if ((l == null || l <= 0) && _pl != null) { try { l = _pl.NextValue(); } catch { } } if (t == null || t <= 0) t = WT(); int coreCount = _sx.Length > 0 ? _sx.Length : Environment.ProcessorCount; if (!includeCores) return new CpuSnapshot(true, coreCount, t, f, l, p, Array.Empty<CoreSnapshot>()); var cores = new CoreSnapshot[_sx.Length]; for (int i = 0; i < _sx.Length; i++) { var (idx, xs) = _sx[i]; float? cF = RV(xs.C), cT = RV(xs.T), cL = RV(xs.L); if ((cF == null || cF <= 0) && f.HasValue) cF = f; if ((cT == null || cT <= 0) && t.HasValue) cT = t; cores[i] = new CoreSnapshot(idx, cF, cT, cL); } return new CpuSnapshot(true, coreCount, t, f, l, p, cores); }
    void Upd() { if (_h != null) { _h.Update(); foreach (var s in _h.SubHardware) s.Update(); } if (!_ok && _sa < 15) { Scan(); _ok = RV(_ts) is > 0 || RV(_cs) is > 0; if (!_ok) _wf = true; } if (_g != null) { _g.Update(); GpuLoadPercent = RV0(_gl); GpuTemperatureC = RV(_gt); GpuVramUsedMB = RV0(_gmu); GpuVramTotalMB = RV(_gmt); if (GpuVramUsedMB.HasValue && GpuVramTotalMB is > 0 && GpuVramUsedMB.Value > GpuVramTotalMB.Value * 1.05f) GpuVramUsedMB = null; } try { long tx = 0, rx = 0; foreach (var ni in System.Net.NetworkInformation.NetworkInterface.GetAllNetworkInterfaces()) { if (ni.OperationalStatus != System.Net.NetworkInformation.OperationalStatus.Up || ni.NetworkInterfaceType == System.Net.NetworkInformation.NetworkInterfaceType.Loopback) continue; var st = ni.GetIPStatistics(); tx += st.BytesSent; rx += st.BytesReceived; } var now = DateTime.UtcNow; double dt = (now - _netT).TotalSeconds; if (dt > 0 && _netTxPrev > 0) { NetUpKBps = (tx - _netTxPrev) / dt / 1024.0; NetDownKBps = (rx - _netRxPrev) / dt / 1024.0; } _netTxPrev = tx; _netRxPrev = rx; _netT = now; } catch { } }
    ...
    static float? RV0(ISensor? s) => s?.Value is >= 0 ? s.Value.Value : null;
    ...
    void ScanGpu() { _g = _c.Hardware.FirstOrDefault(h => h.HardwareType is HardwareType.GpuNvidia or HardwareType.GpuAmd or HardwareType.GpuIntel); if (_g == null) return; _g.Update(); GpuName = _g.Name; var ga = _g.Sensors.ToList(); _gl = ga.FirstOrDefault(s => s.SensorType == SensorType.Load && s.Name.Contains("Core", StringComparison.OrdinalIgnoreCase)) ?? ga.FirstOrDefault(s => s.SensorType == SensorType.Load); _gt = ga.FirstOrDefault(s => s.SensorType == SensorType.Temperature && s.Name.Contains("Core", StringComparison.OrdinalIgnoreCase)) ?? ga.FirstOrDefault(s => s.SensorType == SensorType.Temperature); _gmu = PickGpuMemory(ga, true); _gmt = PickGpuMemory(ga, false); }
    static ISensor? PickGpuMemory(List<ISensor> sensors, bool used)
    {
...
public static class ReportBuilder
{
    ...
    {
        var r = new MachineReport { MachineName = Environment.MachineName, OsVersion = Environment.OSVersion.ToString(), CpuName = cpuName, CoreCount = s.CoreCount, PackageTemperatureC = s.PackageTemperatureC, PackageFrequencyMHz = s.PackageFrequencyMHz, TotalLoadPercent = s.TotalLoadPercent, PackagePowerW = s.PackagePowerW, TimestampUtcMs = DateTimeOffset.UtcNow.ToUnixTimeMilliseconds() };
    ...
    public static CpuDetailReport BuildCpuDetail(CpuSnapshot s, string cpuName)
    {
        var r = new CpuDetailReport { MachineName = Environment.MachineName, CpuName = cpuName, CoreCount = s.CoreCount, PackageTemperatureC = s.PackageTemperatureC, PackageFrequencyMHz = s.PackageFrequencyMHz, TotalLoadPercent = s.TotalLoadPercent, PackagePowerW = s.PackagePowerW, TimestampUtcMs = DateTimeOffset.UtcNow.ToUnixTimeMilliseconds() };
