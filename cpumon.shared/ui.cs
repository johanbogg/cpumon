...
public sealed class FileBrowserDialog : Form
{
    ...
    void ApplyFilter() { var f = _filterBox.Text.Trim(); _fileList.BeginUpdate(); _fileList.Items.Clear(); foreach (var it in _allItems) { var nav = it.Tag as FileNavInfo; if (string.IsNullOrEmpty(f) || nav?.IsUp == true || nav?.IsDrive == true || it.Text.IndexOf(f, StringComparison.OrdinalIgnoreCase) >= 0) _fileList.Items.Add(it); } _fileList.EndUpdate(); }
